...
- **`game-service` (cmd/game)** — Authoritative game state machine + everything else that touches game state. Sync HTTP for the SPA contract: `/api/state`, `/api/games`, `/api/move`, `/api/resign`, `/api/new`, `/api/undo`, `/api/set_players`, `/api/hint`, `/api/replay`, `/api/games/delete`, `/api/invites/*`. Stream consumers: `game:commands` (game-service-group) for engine-translated MakeMove + JoinQueue/LeaveQueue, `engine:results` for engine-worker outputs. Goroutines started at boot: invite expiry sweeper (30s ticker), matchmaker pairing loop (2s ticker, Redis-leader-elected via `mm:leader` so only one replica pairs). Every read-modify-write on `games` rows goes through the per-game Redis lock and the hot cache. **Note:** `/api/touch`, `/api/touch_move`, `/api/assess`, `/api/load`, `/api/save` were removed in the 2026-05-14 cleanup; the Glicko-2 rating updater (`game:events` consumer) was paused 2026-05-15. See `pkg/wire/CONTRACT.md` Section 5 + ROADMAP.md.
