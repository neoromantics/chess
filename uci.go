...
func NewUCI(out io.Writer) *UCI {
	return &UCI{board: StartPosition(), out: out}
}
...
func (u *UCI) startSearch(limits SearchLimits) {
	u.stopSearch()
	...
	go func() {
		defer close(doneCh)
		result := board.IterativeDeepening(limits, stop, func(r SearchResult) {
			u.printInfo(r)
		})
...
func (u *UCI) printInfo(r SearchResult) {
